class InboundController < ApplicationController
  ...
  def receive_receipt_line
    ...
    unless @receipt_lines.empty?
      if @receipt_lines.first.valid_for_receiving?
        @receipt = Receipt.find(:first, params[:receipt_line][:receipt_id])
         @dock_door = DockDoor.find(:first, params[:receipt_line][:dock_door_id])
    ...
    redirect_to :controller =>'inbound', :action => 'confirm_lp', :dock_door_id => @dock_door.id, :receipt_id => @receipt.id
  end
